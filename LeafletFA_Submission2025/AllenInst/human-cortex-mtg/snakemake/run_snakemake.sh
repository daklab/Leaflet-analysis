...
#SBATCH --output=HUMAN_ALLEN_MTG_%j.log
...
slurm_out=/gpfs/commons/datasets/controlled/BRAIN_NeMO/human-cortex-mtg/slurm
...
snakemake -j 64 --cluster-config cluster.json --cluster "sbatch -N 1 -p cpu -c {cluster.cpus} --mem={cluster.mem} -t {cluster.time} -J {cluster.job-name} --output=$slurm_out_today/slurm-%j.out --error=$slurm_out_today/slurm-%j.err" --latency-wait 120 --rerun-incomplete #--unlock
echo "Snakemake workflow submitted"

# cd /gpfs/commons/home/kisaev/Leaflet-analysis/AllenInst/human-cortex-mtg/snakemake
# sbatch /gpfs/commons/home/kisaev/Leaflet-analysis/AllenInst/human-cortex-mtg/snakemake/run_snakemake.sh 