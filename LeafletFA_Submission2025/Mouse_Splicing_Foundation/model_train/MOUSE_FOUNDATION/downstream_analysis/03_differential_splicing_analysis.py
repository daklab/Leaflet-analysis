...
if len(sys.argv) > 1:
    ...
    print(f"Using specified MODEL_  OUTPUTS_DIR: {MODEL_OUTPUTS_DIR}")
...
def main():
    ...
    model_path = os.path.join(MODEL_OUTPUTS_DIR, f"run_{param_id}", "leafletfa_model.pkl.gz")
    ...
    def analyze_celltype_subclustering_with_features(splice_adata, ge_adata, PLOTS_DIR=None, DATA_DIR=None, 
        ...
        feature_sets = {
            ...
            'Combined': np.hstack([splice_adata.obsm["X_PHI"], ge_adata.obsm["X_scVI_linear"]])
        }
    ...
    def analyze_subcluster_feature_importance(splice_adata, ge_adata, subclustering_results,
        ...
        nmf_features = ge_adata.obsm["X_scVI_linear"]
        ...
        feature_names = ([f"Splice_Factor_{i}" for i in range(n_splice)] + 
                        [f"scVI_Latent_{i}" for i in range(n_nmf)])
        ...
        for cell_type, results in subclustering_results.items():
            ...
            importance_df = pd.DataFrame({
                ...
                'feature_type': (['Splicing'] * n_splice + ['Gene_Expression_scVI'] * n_nmf)
