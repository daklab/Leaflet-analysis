#!/usr/bin/env python
"""
LeafletFA Input Preparation - Mouse Splicing Foundation
...
# Input file paths
SPLICE_INPUT = "/gpfs/commons/groups/knowles_lab/Karin/Leaflet-analysis-WD/MOUSE_SPLICING_FOUNDATION/MODEL_INPUT/072025/aligned_splicing_data_20250707_181219.h5ad"
...
ATSE_FILTER_PERCENTILE = 0.55  # Filter out ATSEs below this percentile
...
def compute_dimensionality_reduction(splice_adata):
    ...
    try:
        ...
        print("   ⚙️ Using pre-calculated centered PSI values...")
        
        # Perform PCA using sparse data
        print(f"   ⚙️ Computing PCA with {N_PCA_COMPONENTS} components...")
...
def identify_waypoints_and_metacells(splice_adata):
    ...
    try:
        ...
        print(f"   ⚙️ Finding {N_WAYPOINTS} waypoints from the PCA components...")
        ...
        print(f"   ⚙️ Assigning {METACELL_SIZE} nearest cells to each waypoint...")
...
def generate_and_store_initializations(splice_adata, waypoints_dict, metacell_dicts):
    ...
    try:
        ...
        print("   ⚙️ Computing Phi and Psi initializations...")
        ...
        for i, n_waypoints in enumerate(waypoints_dict.keys()):
            print(f"   ⚙️ Storing initializations for {n_waypoints} waypoints...")
...
def save_prepared_anndata(splice_adata):
    ...
    try:
        ...
        output_filename = f"MOUSE_SPLICING_FOUNDATION_Anndata_ATSE_counts_{num_junctions}_junctions_{waypoint_str}_waypoints_{timestamp}.h5ad"
