...
# Define output directory (organized by cell type)
cell_type_clean = cell_type.replace(' ', '_').replace('/', '_')
output_dir = os.path.join(base_output_dir, "results", cell_type_clean, f"run_{param_id}")
...
# Initialize wandb
wandb.init(
    project="LeafletFA-MouseFoundation-CellType",  # Updated project name
    config=params,  # Config parameters for this run
    # add time and cell type to run name 
    name=f"{cell_type_clean}_run_{param_id}_{today}",  # Name of this run
    dir=output_dir,  # Directory to store wandb files
    # Optional: Add a group for easier organization
    group=f"MouseFoundation_{cell_type_clean}",
    # Optional: Add notes
    notes=f"Cell type: {cell_type}, Parameter set {param_id}, K={params['K']}, waypoints={params['waypoints_use']}"
)
...
wandb.config.update({
    "param_id": param_id,
    "cell_type": cell_type,
    "data_source": "MouseFoundation_CellType",
